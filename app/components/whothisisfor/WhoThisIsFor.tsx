...
const WhoThisIsFor = () => {
  ...
  return (
    <section className="text-center py-16 bg-gradient-to-b from-white to-blue-50">
      <h2 className="text-3xl font-bold mb-2">👩‍🏫 Who This Is For</h2>
      <p className="text-gray-600 mb-12">Designed for every member of the school community</p>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto px-4">
        {roles.map((role, index) => (
          <RoleCard key={index} emoji={role.emoji} title={role.title} features={role.features} />
        ))}
      </div>
    </section>
  );
